...
public class TileEntityInfusionFucker extends TileEntity implements IAspectContainer, IEssentiaTransport {
    ...
    public void matrixFuckery() {
        ...
        /*
         * this method handles the Aspect array that I use to handle suction, if the matrix's AspectList has anything in
         * it, the Aspect array is set to be a mirrored copy of it, otherwise, it sets the interceptor's internal
         * AspectList to a new blank AspectList so that it can be reused without breaking.
         */
        setTodoList();
        ...
        if (!grabbedAspects.equals(new AspectList())) {
            ...
            try {
                ...
                } else if (listSlot == listOfAspects.length) {
                    resetLists();
                }
            } catch (Exception ignored) {}
    ...
    /**
     * if the matrix has an AspectList active, then it sets the Aspect array so that suction can be set, otherwise it
     * resets the interceptor's AspectList into a clean list.
     */
    ...
    /**
     * This methods Takes into consideration if the suction is null, and sets it if it can, otherwise, if the
     * interceptor AspectList is equal to the grabbedAspects AspectList of the matrix, it tiers up the listSlot
     * iteration int and removes the essentia from the infusion matrix while setting the suction to null so the next
     * tick can set the suction to the new aspect.
     */
    private void setInterceptorSuction() {
        if (currentSuction == null) {
            this.currentSuction = listOfAspects[listSlot];
        } else if (myAspects.getAmount(listOfAspects[listSlot]) >= grabbedAspects.getAmount(listOfAspects[listSlot])) {
            ...
            if (essentiaEntity != null) {
                if (essentiaEntity.getEssentiaAmount(ForgeDirection.UP) >= 1) {
                    ...
                    if (tubeAspect.equals(currentSuction)) {
                        essentiaEntity.takeEssentia(tubeAspect, 1, ForgeDirection.UP);
                        myAspects.add(tubeAspect, 1);
                    }
                }
            }
        } catch (Exception ignored) {}
